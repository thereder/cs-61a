...
def scheme_eval(expr, env):
    """Evaluate Scheme expression EXPR in en vironment ENV.
...
def apply_primitive(procedure, args, env):
    ...
    try:
        ...
        if procedure.use_env:
            args_py.append(env)
        return procedure.fn(*args_py)
    except TypeError:
        raise SchemeError("Unsupported type")
...
class Frame:
    ...
    def make_call_frame(self, formals, vals):
        ...
        while len(formals) > 0:
            frame.bindings[formals[0]] = vals[0]
...
def do_lambda_form(vals, env):
    ...
    if vals.second.second != nil:
        vals.second.first = Pair("begin", Pair(vals.second.first, vals.second.second))
    return LambdaProcedure(vals.first, vals.second.first, env)
...
def do_mu_form(vals):
    ...
    if vals.second.second != nil:
        vals.second.first = Pair("begin", Pair(vals.second.first, vals.second.second))
    return MuProcedure(vals.first, vals.second.first)
...
def do_or_form(vals, env):
    ...
    while len(vals) > 1:
        if scheme_true(scheme_eval(vals[0], env)):
            return scheme_eval(quote(vals[0]), env)
        vals = vals.second
    return vals[0]
...
def check_formals(formals):
    ...
    if len(formals) == 0:
        return set()
    checked = check_formals(formals.second)
    if not scheme_symbolp(formals[0]):
        raise SchemeError("bad symbol")
    elif formals[0] in checked:
        raise SchemeError("repeated symbol")
    else:
        checked.add(formals[0])
        return checked
