...
(define (merge comp list1 list2)
  ; *** YOUR CODE HERE ***
  (if (and (= 0 (length list1)) (= 0 (length list2)))
    nil
    (if (and (> (length list1) 0) (> (length list2) 0))
      (if (or (comp (car list1) (car list2)) (= 0 (length list2)))
...
(define (helper total max-pieces max-value previous)
  (if (= total 0)
    (cons previous nil)
    (if (= max-pieces 0)
      nil
      (if (< total 0)
        ...
        (if (= 0 max-value)
          nil
          (append
            (helper (- total max-value) (- max-pieces 1) max-value (cons max-value previous))
            (helper total max-pieces (- max-value 1) previous)
          )
        )
      )
    )
...
(define (tree-sums tree)
  ; *** YOUR CODE HERE ***
  ; (I did not see the map, filter and accumulate functions before i implemented it, 
  ; but it's good practice to write my own so I decide to leave it here)
  (if (= (length (children tree)) 0)
    (cons (entry tree) nil)
    (add-entry (entry tree) (form-result (children tree)))
  )
)

(define (add-entry entry result)
  (if (= (length result) 0)
    nil
    (cons (+ entry (car result)) (add-entry entry (cdr result)))
  )
)

(define (form-result children)
  (if (= (length children) 0)
    nil
    (append (tree-sums (car children)) (form-result (cdr children)))
  )
)
