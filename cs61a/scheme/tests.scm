...
(define (pow b n)
  (if (= 1 n) b
    (if (even? n)
      (pow (square b) (floor (/ n 2)))
      (* b (pow (square b) (floor (/ n 2))))
    )  
  )
)
...
(define (dog bird)
  (define (cow tweet moo)
    (define woof (bird tweet))
    (print moo)
    woof
  )
  cow
...
(define (fox_says start middle end num)
  (define (repeat k)
    (if (= 1 k)
      (list middle)
      (append (list middle '-) (repeat (- k 1)))
    )
  )
  (append 
    (append 
...
(fox_says 'wa 'pa 'pow 5)
; expect (wa - pa - pa - pa - pa - pa - pow)
...
(assert-equal (cons 1 (cons 2 (cons 3 (cons 4 nil)))) (list 1 2 3 4))
; expect okay
...
(define (partition number)
  (if (= 1 number)
    (cons (cons 1 nil) nil)
    (begin
      (define (single-call x)
        (define (add-x lst)
          (if (= 0 (length lst))
            nil
            (cons 
              (cons (- number x) (car lst)) 
              (add-x (cdr lst))
            )
          )
        )
...
(define (merge-lists lst)
  (if (= 0 (length lst))
    nil
    (append (car lst) (merge-lists (cdr lst)))
  )
)
...
(define (range lower upper)
  (if (< upper lower)
    nil
    (cons lower (range (+ lower 1) upper))
  )
)

; The following two functions are adopted from questions.scm
...
(partition 4)
; expect ((4) (3 1) (2 2) (2 1 1) (1 3) (1 2 1) (1 1 2) (1 1 1 1))
