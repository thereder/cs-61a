    ...
    def remove_insect(self, insect):
        ...
        if not insect.is_ant():
            ...
            if isinstance(insect, BodyguardAnt):
                self.ant = insect.ant
            elif isinstance(insect, QueenAnt) is False or insect.is_true is False:
                self.ant = None
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        current_position = 0
        place = self.place
        while current_position < self.min_range and place is not None:
            place = place.entrance
            current_position += 1
        for i in range(self.max_range-self.min_range+1):
            if place is not None:
                if len(place.bees) > 0 and place is not hive:
...
class QueenAnt(ScubaThrower):
    ...
    def action(self, colony):
        ...
        if self.is_true == False:
            ...
            """Start from one end of the tunnel and double damage all the way to the other end"""
            place = self.place
            while place.entrance is not None and place.entrance.exit is not None:
                place = place.entrance
            while place is not None:
                if isinstance(place.ant, QueenAnt) is False and place.ant is not None:
                    if place.ant not in self.doubled_ants:
                        place.ant.damage *= 2
                        self.doubled_ants.append(place.ant)
                if isinstance(place.ant, BodyguardAnt):
                    if place.ant.ant is not None and not isinstance(place.ant.ant, QueenAnt):
