...
def interpret_mobile(s):
    ...
    for t in ('Branch', 'Weight', 'Mobile'):
        "*** YOUR CODE HERE ***"
        result = interpret_mobile(s[:next_T]+t+s[next_T+1:])
        if result:
            return result
    return None
...
class Stream:
    ...
    def __iter__(self):
        ...
        """
        "*** YOUR CODE HERE ***"
        def stream_generator(next):
            while next is not Stream.empty:
                yield next.first
                next = next.rest
...
def merge(s0, s1):
    ...
    def compute_rest():
        if e0 < e1:
            ...
            return merge(s0, s1.rest)
        else:
            return merge(s0.rest, s1.rest)
    if e0 < e1:
        return Stream(e0, compute_rest)
    else:
        return Stream(e1, compute_rest)
...
def make_s():
    ...
    def rest():
        "*** YOUR CODE HERE ***"
        return merge(scale_stream(make_s(), 2), merge(scale_stream(make_s(), 3), scale_stream(make_s(), 5)))
    s = Stream(1, rest)
    return s
