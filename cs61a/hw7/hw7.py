...
class MinimalAmount(Amount):
    ...
    "*** YOUR CODE HERE ***"
    def __init__(self, nickels, pennies):
        Amount.__init__(self, nickels + pennies // 4, pennies - (pennies // 4 * 5))
...
def apply(operator_name, shape):
    ...
    apply.implementation[('area', 'square')] = lambda x: x.side * x.side
    apply.implementation[('area', 'rect')] = lambda x: x.width * x.height
    apply.implementation[('perimeter', 'square')] = lambda x: x.side * 4
    apply.implementation[('perimeter', 'rect')] = lambda x: (x.width + x.height) * 2
    return apply.implementation[(operator_name, type_tag(shape))](shape)     

def type_tag(an_object):
    if isinstance(an_object, Square):
        return 'square'
    if isinstance(an_object, Rect):
        return 'rect'
...
def adjoin_set2(s, v):
    ...
    if s.rest is Rlist.empty:
        if s.first == v:
            ...
            return Rlist(s.first, Rlist(v) )
    elif v < s.first:
        return Rlist(v, s)
    elif v == s.first:
        return adjoin_set2(s.rest, v)
    else:
...
def union_set2(set1, set2):
    ...
    if empty(set2):
        ...
        return union_set2(adjoin_set2(set1, set2.first), set2.rest)
...
def tree_to_ordered_sequence(s):
    ...
    >>> tree_to_ordered_sequence(big_tree(0, 15))
    Rlist(0, Rlist(2, Rlist(4, Rlist(7, Rlist(9, Rlist(12, Rlist(14)))))))
    """
    "*** YOUR CODE HERE ***"
    tree_list = []
    result = Rlist.empty
    def tree_to_list(tree, lst):
        if tree is not None:
            ...
            tree_to_list(tree.left, lst)
            tree_to_list(tree.right, lst)
    tree_to_list(s, tree_list)
    tree_list.sort()
    tree_list.reverse()
    for e in tree_list:
        result = Rlist(e, result)
    return result
...
def has_cycle(s):
    ...
    def true_false(rlst):
        if rlst == Rlist.empty:
            ...
            return true_false(rlst.rest)
    return true_false(s.rest)
